/**********************************************************
 *
 *  DEMAND_CHB.ADO: Value of the demand equation if the entrant is a Republican minus the value if the entant is a Democrat
 *  
 ...
 * bt (parameter to estimate), G (parameter to estimate), and a (a constant value) .
 *
...
program define Gcom_rplus
	version 11
			if ($r+$d)>=2{
				replace Gcomb = comb($r+$d,2)*G if ((nr+1)>=$r)*(nd>=$d)==1
			}
			else{
				replace Gcomb=0 if ((nr+1)>=$r)*(nd>=$d)==1
			}		
end
...
program define Gcom_dplus
	version 11
			if ($r+$d)>=2{
					replace Gcomb = comb($r+$d,2)*G if (nr>=$r)*((nd+1)>=$d)==1
				}
				else{
					replace Gcomb=0 if (nr>=$r)*((nd+1)>=$d)==1
				}
end
 
 
program define demand_chb
	version 11
	syntax
