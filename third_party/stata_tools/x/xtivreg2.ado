...
di as err "Error - must have ivreg2/ivreg28/ivreg29 version 2.1.15 or greater installed"
				exit 601
			}
		}
		local vernum "`e(version)'"
...
* Do this even though ivreg2 currently won't accept TS operators with partial()
			...
			`ivreg2_cmd' `lhs_fd' `inexog_fd' (`endo_fd' = `exexog_fd') `wtexp' `if', /*
				*/	`first' `ffirst' `rf' /*
				*/	savefprefix(`savefprefix') saverfprefix(`saverfprefix') /*
				*/	cluster(`cluster') orthog(`orthog_fd') endog(`endogtest_fd') /*
				*/	 redundant(`redundant_fd') partial(`partial_fd') tvar(`tvar') bw(`bw') `options'
			preserve
			tempvar touse
...
* Only iw and fw use weighted observation counts
			...
			by `ivar' `touse' : replace `T_i'=`T_i'[_N] if `touse' & _n<_N
...
* This guarantees that the demeaned variables are doubles
				...
				`ivreg2_cmd' `lhs_t' `inexog_t' (`endo_t' = `exexog_t') `wtexp' if `touse', /*
