...
void mixl_ll(string scalar B_s)
{
	...
	corr = mixl_corr
	wgttyp = mixl_wgttyp
	accuracy = mixl_accuracy
	sgi = mixl_sgi

	B = st_matrix(B_s)'
	if (sgi == 1) {
	nw = nwspgr("KPN", krnd, accuracy)
	ERR = nw[.,1..krnd]'
	WT = nw[.,(krnd+1)]'
	nrep = rows(nw)
	}
	...
	i = 1
	for (n=1; n<=np; n++) {
		if (sgi == 0) ERR = invnormal(halton(nrep,krnd,(1+burn+nrep*(n-1)))')
